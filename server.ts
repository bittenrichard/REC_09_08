...
console.log(`[DIAGNÓSTICO] Node ENV: ${process.env.NODE_ENV || 'development'}`);
console.log(`[DIAGNÓSTICO] GOOGLE_CLIENT_ID (lido): ${GOOGLE_CLIENT_ID.substring(0, 5)}...${GOOGLE_CLIENT_ID.substring(GOOGLE_CLIENT_ID.length - 5)}`);
console.log(`[DIAGNÓSTICO] GOOGLE_CLIENT_SECRET (lido): ${GOOGLE_CLIENT_SECRET.substring(0, 5)}...${GOOGLE_CLIENT_SECRET.substring(GOOGLE_CLIENT_SECRET.length - 5)}`);
console.log(`[DIAGNÓSTICO] GOOGLE_REDIRECT_URI (lido): ${GOOGLE_REDIRECT_URI}`);
console.log("============================================================================");
...
app.post('/api/upload-curriculums', upload.array('curriculumFiles'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (N8N_TRIAGEM_WEBHOOK_URL) {
      ...
      if (newCandidateEntries.length > 0 && jobInfo && userInfo) {
...
app.get('/api/google/auth/callback', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Este bloco de erro agora é mais robusto para NUNCA mais travar o servidor.
